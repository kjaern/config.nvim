return {
	...
	config = function()
		local function shorter_name(filename, source)
			return "." .. string.gsub(filename, vim.loop.cwd(), "")
		end
		...
		require("venv-selector").setup({
			...
			settings = {
				...
				search = {
					conda = {
						command = "fd conda//python.exe$ --full-path -IH -a --color never",
						type = "anaconda",
	...
	keys = {
		...
		{ "<leader>vc", "<cmd>VenvSelectCached<cr>" },
		{
			...
			desc = "Python venv: select python env",
		},
	},
}
